...
//   const { updateNodeData, getEdges, getNodes, getNode, getEdge, updateEdgeData } = useReactFlow<
...
//     e.stopPropagation();
//     propagatePortValues(data.id, data.id, !portState);
//   };

//   const propagatePortValues = (portId: string, nodeId: string, portValue: boolean) => {
//     // update port value
...
//       updateEdgeData(edge.id, { value: portValue });
//     }

//     // update output of nodes connected to the edges
//     for (const edge of currentEdges) {
...
//           propagatePortValues(outputPort.id, nodeId, outputPort.value);
//         }
//       }
//     }
//   };
...
//     const inputPorts = currentChip.data.ports?.filter((port) => port.type === NodeType.IN);
//     const outputPorts = currentChip.data.ports?.filter((port) => port.type === NodeType.OUT);
...
//       if (internalPort.type === NodeType.OUT) continue;
...
//     const internalInputPorts = internalPorts.filter((port) => port.type === NodeType.IN);
